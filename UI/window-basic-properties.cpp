	  ...
	  buttonBox              (new QDialogButtonBox(this))
{
	...
	if (drawable_preview && drawable_type) {
		...
		obs_source_inc_showing_main(sourceClone);
...
OBSBasicProperties::~OBSBasicProperties()
{
	if (sourceClone) {
		...
		obs_source_dec_showing_main(sourceClone);
	}
		...
		uint32_t cy)
{
	...
	bool stillTransitioning = true;
			//obs_transition_get_time(window->sourceClone) < 1.0f;
	if (!stillTransitioning) {
		OBSSource start;
		OBSSource end;

		if (window->direction) {
			start = window->sourceA;
			end = window->sourceB;
		} else {
			start = window->sourceB;
			end = window->sourceA;
		}

		obs_transition_set(window->sourceClone, start);
		obs_transition_start(window->sourceClone,
				OBS_TRANSITION_MODE_AUTO,
				window->main->GetTransitionDuration(), end);
		window->direction = !window->direction;
