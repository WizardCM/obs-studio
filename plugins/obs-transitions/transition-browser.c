...
/**
 * TODO:
 * - DispatchJSEvent on transition start/stop
 *    - Start: provide transition duration information
 * - Toggle webpage visibility on transition start/stop
 *    - Currently the page is considered "hidden"
 * - Strip out remaining Stinger-related code
 * - Fix not-working transition point (expose a function for the webpage to control?) maybe via browser_video_render
 * - Switching between transitions seems to break this transition completely (sometimes?)
 */
...
static const char *browser_get_name(void *type_data)
{
...
static void browser_update(void *data, obs_data_t *settings)
{
	struct obs_video_info ovi;
	obs_get_video_info(&ovi);

	struct browser_info *s = data;
	const char *path = obs_data_get_string(settings, "url");

	obs_data_t *browser_settings = obs_data_create();
	obs_data_set_string(browser_settings, "url", path);
	obs_data_set_int(browser_settings, "width", ovi.base_width);
	obs_data_set_int(browser_settings, "height", ovi.base_height);

	obs_source_release(s->browser_source);
	s->browser_source = obs_source_create_private("browser_source", NULL,
						      browser_settings);
	...
	obs_source_set_monitoring_type(s->browser_source, s->monitoring_type);
...
{
	struct browser_info *s = bzalloc(sizeof(*s));

	s->source = source;
	s->mix_a = mix_a_fade_in_out;
	s->mix_b = mix_b_fade_in_out;

	// This function overrides the normal duration length, uncomment if you want the webpage to control duration
	// obs_transition_enable_fixed(s->source, true, 0);
	obs_source_update(source, settings);
	return s;
}

static void browser_destroy(void *data)
{
	struct browser_info *s = data;
	obs_source_release(s->browser_source);
	bfree(s);
...
static void browser_video_render(void *data, gs_effect_t *effect)
{
	struct browser_info *s = data;
	...
	uint32_t media_cx = obs_source_get_width(s->browser_source);
	uint32_t media_cy = obs_source_get_height(s->browser_source);
	...
	obs_source_video_render(s->browser_source);
	gs_matrix_pop();
...
static bool browser_audio_render(void *data, uint64_t *ts_out,
				 struct obs_source_audio_mix *audio,
				 uint32_t mixers, size_t channels,
				 size_t sample_rate)
{
	// TODO: No idea if this even works with browser audio yet
	struct browser_info *s = data;
	uint64_t ts = 0;

	if (!obs_source_audio_pending(s->browser_source)) {
		ts = obs_source_get_audio_timestamp(s->browser_source);
		if (!ts)
			return false;
	}
	...
	obs_source_get_audio_mix(s->browser_source, &child_audio);
...
static void browser_transition_start(void *data)
{
	// TODO: Send transition_start event to webpage
	struct browser_info *s = data;
	...
	if (s->browser_source) {
		...
		proc_handler_t *ph =
			obs_source_get_proc_handler(s->browser_source);
		...
		// See other mention of this function
		/* obs_transition_enable_fixed(
			s->source, true, (uint32_t)(s->duration_ns / 1000000)); */

		calldata_free(&cd);

		obs_source_add_active_child(s->source, s->browser_source);
	}

	s->transitioning = true;
}

static void browser_transition_stop(void *data)
{
	// TODO: Send transition_stop event to webpage
	struct browser_info *s = data;

	if (s->browser_source)
		obs_source_remove_active_child(s->source, s->browser_source);

	s->transitioning = false;
}

static void browser_enum_active_sources(void *data,
					obs_source_enum_proc_t enum_callback,
					void *param)
{
	struct browser_info *s = data;
	if (s->browser_source && s->transitioning)
		enum_callback(s->source, s->browser_source, param);
}

static void browser_enum_all_sources(void *data,
				     obs_source_enum_proc_t enum_callback,
				     void *param)
{
	struct browser_info *s = data;
...
static obs_properties_t *browser_properties(void *data)
{
	obs_properties_t *ppts = obs_properties_create();

	obs_properties_set_flags(ppts, OBS_PROPERTIES_DEFER_UPDATE);

	// TODO: Local file support. See `obs-browser-plugin.cpp` for reference
	/* obs_properties_add_bool(
		ppts, "is_local_file", obs_module_text("LocalFile")); */
	obs_properties_add_text(ppts, "url", obs_module_text("URL"),
				OBS_TEXT_DEFAULT);
	/* obs_properties_add_path(ppts, "local_file", obs_module_text("VideoFile"),
...
struct obs_source_info browser_transition = {
	.id = "obs_browser_transition",
	.type = OBS_SOURCE_TYPE_TRANSITION,
	.get_name = browser_get_name,
	.create = browser_create,
	.destroy = browser_destroy,
	.update = browser_update,
	.video_render = browser_video_render,
	.audio_render = browser_audio_render,
	.get_properties = browser_properties,
	.enum_active_sources = browser_enum_active_sources,
	.enum_all_sources = browser_enum_all_sources,
